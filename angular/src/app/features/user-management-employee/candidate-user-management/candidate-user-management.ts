...
@Component({
  selector: 'app-candidate-user-management',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './candidate-user-management.html',
  styleUrls: ['./candidate-user-management.scss']
})
export class CandidateUserManagementComponent implements OnInit, OnDestroy {
  // Sidebar state
  sidebarWidth = 72;
  ...
  filteredUsers: CandidateUser[] = [];
  paginatedUsers: CandidateUser[] = [];
  ...
  selectedUser: CandidateUser | null = null;
  ...
  private getSidebarWidth(): number {
    ...
    const pageElement = document.querySelector('.candidate-user-management-page');
  ...
  // Actions
  onForceLogout(user: CandidateUser): void {
  ...
  onEnforcePasswordChange(user: CandidateUser): void {
    this.selectedUser = user;
  ...
  onAddIpRestrict(user: CandidateUser): void {
    this.selectedUser = user;
    this.ipAddress = '';
  ...
  onRemoveIp(user: CandidateUser, ip: string): void {
  ...
  // Helper methods
  getStatusLabel(user: CandidateUser): string {
    if (user.isLocked) return 'Đã khóa';
    if (!user.isActive) return 'Ngừng hoạt động';
    return 'Đang hoạt động';
  }

  getStatusClass(user: CandidateUser): string {

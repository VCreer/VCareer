...
namespace VCareer.Repositories.Job
{
    public class JobPostRepository : EfCoreRepository<VCareerDbContext, Job_Post, Guid>, IJobPostRepository
    {
        ...
        private IQueryable<Job_Post> ApplyIncludes(IQueryable<Job_Post> query, bool includeDetails)
        {
            if (!includeDetails)
                return query;
        ...
        private IQueryable<Job_Post> BuildActiveJobQuery(IQueryable<Job_Post> query)
        {
            return query
        ...
        // lấy job thoe slug
        public async Task<Job_Post> GetBySlugAsync(string slug, bool includeDetails = true)
        {
        ...
        public async Task<List<Job_Post>> GetByIdsAsync(List<Guid> ids, bool includeDetails = true)
        {
            if (ids == null || !ids.Any())
        ...
        public async Task<List<Job_Post>> GetJobsByIdsAsync(List<Guid> jobIds)
        {
            if (!jobIds.Any())
                return new List<Job_Post>();
        ...
        public async Task<List<Job_Post>> GetAllActiveJobsAsync()
        {
            var dbContext = await GetDbContextAsync();
